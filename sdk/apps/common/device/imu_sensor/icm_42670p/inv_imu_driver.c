...
int inv_imu_get_data_from_fifo(struct inv_imu_device *s)
{
    ...
    if ((int_status & INT_STATUS_FIFO_THS_INT_MASK)
        ...
        while (packet_count > 0) {
            ...
            packet_count_i = 0;
            u16 packet_tmep = 1024 / get_package_size();
            if (packet_count > packet_tmep) {//1k/6/2
                packet_count_i = packet_count - packet_tmep;
            }
            for (; packet_count_i < packet_count; packet_count_i++) {
                inv_imu_sensor_event_t event;
                event.sensor_mask = 0;
                header = (fifo_header_t *) &s->fifo_data[fifo_idx];
                fifo_idx += FIFO_HEADER_SIZE;
                /* printf("head:0x%x, %d\n", header->Byte, packet_count_i); */
...
int inv_imu_configure_fifo(struct inv_imu_device *s, INV_IMU_FIFO_CONFIG_t fifo_config)
{
    ...
    case INV_IMU_FIFO_ENABLED :
        ...
        */
        ...
        data = fifo_pack_num;//10包
        status |= inv_imu_write_reg(s, FIFO_CONFIG2, 1, &data);
        /* u16 fifo_wm = 10;//10包 */
        /* u8 temp_data[2]; */
        /* temp_data[0] = fifo_wm &0xff; */
        /* temp_data[1] = (fifo_wm>>8) &0x0f; */
        /* status |= inv_imu_write_reg(s, FIFO_CONFIG2, 2, temp_data); */
...
int inv_imu_enter_sleep(struct inv_imu_device *s)
{
    ...
    //1.close rcosc.fifo config6:addr0x02 bit0=1
    status |= inv_imu_read_reg(s, FIFO_CONFIG6_MREG1, 1, &value);
    value |= FIFO_CONFIG6_RCOSC_REQ_ON_FIFO_THS_DIS_MASK;
    status |= inv_imu_write_reg(s, FIFO_CONFIG6_MREG1, 1, &value);
    ...
    value &= 0xe0; //close acc,gpro,idle
    status |= inv_imu_write_reg(s, PWR_MGMT0, 1, &value);
    inv_imu_sleep_us(20000);
...
int inv_imu_exit_sleep(struct inv_imu_device *s)
{
    ...
    /* set FIFO packets to 20bit format (i.e. high res is enabled) */
    status |= inv_imu_read_reg(s, PWR_MGMT0, 1, &value);
    value |= 0x0f; //close acc,gpro,idle
    status |= inv_imu_write_reg(s, PWR_MGMT0, 1, &value);
    inv_imu_sleep_us(50000);

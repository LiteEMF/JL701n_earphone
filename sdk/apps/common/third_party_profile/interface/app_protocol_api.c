...
#endif
#if APP_PROTOCOL_GFPS_CODE
//u8 gfps_tws_support = 1;
static const struct callback_register_t gfps_callback = {
    .message_handler_regedit = NULL,
    .check_some_status_register = NULL,
    .tx_resume = NULL,
    .rx_resume = NULL,
    .get_battery = NULL,
};
static uint8_t google_model_id[3] = {0xF2, 0x63, 0x8D};
static char google_public_key[64] = {
...
static char google_private_key[32] = {
...
static int app_protocol_gfps_init(int match_id)
{
    ...
    if (app) {
        ...
        if (app) {
            ...
            app->loop = NULL;
            app->tx_loop = NULL;
            app->special_message = NULL;
            app->settings = &gfps_settings;
            app->app_ctrl = &gfps_ctrl;
            app->callback = &gfps_callback;
            gfps_set_anti_spoofing_public_key(google_public_key);
            gfps_set_anti_spoofing_private_key(google_private_key);
            gfps_set_model_id(google_model_id);
            app_protocol_common_init(app);
